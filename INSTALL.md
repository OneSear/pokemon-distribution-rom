...
These instructions explain how to set up the tools required to build the distribution ROM, including [**rgbds**](https://github.com/gbdev/rgbds), which assembles the source files into a ROM.

If you run into trouble with the toolchain itself, the pret community can help: [Discord](https://discord.gg/d5dubZ3) or [libera#pret](https://web.libera.chat/?#pret).
...
Now you're ready to [build the ROM](#build-the-distribution-rom).
...
## Build the distribution ROM

To download the source files:

```bash
git clone git@github.com:OneSear/pokemon-distribution-rom.git
cd pokemon-distribution-rom
```

To build **pokemon_distribution.gb**:
...
If you have different projects that require different versions of `rgbds`, it might not be convenient to install rgbds 0.9.3 globally. Instead, you can put its files in a directory within the project, such as `pokemon-distribution-rom/rgbds-0.9.3/`. Then specify it when you run `make`:
